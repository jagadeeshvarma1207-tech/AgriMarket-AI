    ...
    <div className="dashboard-layout">
      ...
      <main className="dashboard-main">
        <div className="dashboard-header">
          <div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.2rem', margin: 0 }}>Dashboard Overview</h1>
            <p style={{ color: 'var(--color-text-muted)', fontSize: '0.8rem', margin: 0 }}>Welcome back, farmer</p>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 20, marginBottom: 32 }}>
                {[
                  { label: 'Active Products', value: stats.activeProducts, total: `${stats.totalProducts} total`, icon: Package, color: '#16a34a', bg: 'rgba(22,163,74,0.15)' },
                  { label: 'Pending Orders', value: stats.pendingOrders, total: `${stats.totalOrders} total`, icon: Clock, color: '#f59e0b', bg: 'rgba(245,158,11,0.15)' },
                  { label: 'Total Revenue', value: formatPrice(stats.totalRevenue), total: `${stats.completedOrders} completed`, icon: TrendingUp, color: '#3b82f6', bg: 'rgba(59,130,246,0.15)', isPrice: true },
                  { label: 'Avg. Rating', value: stats.averageRating > 0 ? stats.averageRating.toFixed(1) : '—', total: `${stats.reviewCount} reviews`, icon: Star, color: '#f59e0b', bg: 'rgba(245,158,11,0.15)' },
                ].map((s) => (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 24 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  <div className="card" style={{ padding: 20 }}>
                    <h4 style={{ fontFamily: 'var(--font-display)', marginBottom: 16, fontSize: '1rem' }}>Quick Actions</h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {[
                        { label: 'Add New Product', href: '/farmer/products/new', color: 'btn-primary' },
                        { label: 'Run AI Quality Check', href: '/farmer/ai-quality', color: 'btn-ai' },
                        { label: 'View Orders', href: '/farmer/orders', color: 'btn-outline' },
                        { label: 'Update Location', href: '/farmer/location', color: 'btn-ghost' },
                      ].map((a) => (
